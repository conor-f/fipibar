...
class FipibarClient:
            ...
            return (
                int(
                    subprocess.check_output(
                        "ps aux | grep "
                        + self.unique_fip_process_string
                        + " | grep -v grep | grep -v python | wc -l",
                        shell=True,
                    )
                )
                >= 1
    ...
    def play(self):
        subprocess.check_output(
            ...
            + self.unique_fip_process_string
            + " ffplay -nodisp "
            + self.stations_list[self.config.get("current_station", 0)]["stream_url"]
            + ' > /dev/null 2>&1 &"',
            shell=True,
        )
    ...
    def get_currently_playing_string(self):
        ...
        try:
            ...
            data = requests.get(
                self.stations_list[self.config.get("current_station", 0)]["details_url"]
            ).json()
            ...
            if any(
                ...
                if self.config.get("lastfm_should_scrobble", False):
                    ...
                    self.lastfm_client.update_now_playing(
                        artist=current_artist_name, title=current_track_name
                    )
            ...
            if len(currently_playing_string) > self.currently_playing_trunclen:
                return (
                    currently_playing_string[: self.currently_playing_trunclen - 3]
                    + "..."
                )
            else:
        ...
        if self.config.get("should_heart_on_lastfm", False):
            try:
                ...
                self.lastfm_client.get_track(
                    current_artist_name, current_track_name
                ).love()
            except Exception as e:
        ...
        if self.config.get("should_add_to_spotify", False):
            try:
                ...
                playlist_id = self.spotibar_client.get_playlist_id_from_name(
                    "~F~I~P~", create_if_empty=True
                )

                self.spotibar_client.add_track_to_playlist(playlist_id, track_id)
            except Exception as e:
